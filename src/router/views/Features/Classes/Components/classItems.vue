...
import { classesMethods } from '@state/helper/classes'
...
export default {
  ...
  methods: {
    ...classesMethods,
    itemClicked(evt){
      const selectedClass = {...evt.currentTarget.dataset}
      const filteredClass = this.classes.filter(classItem => classItem.rosterId === selectedClass.rosterId)
      if(!filteredClass.length) return

      const classItem = filteredClass[0];
      this.selectClass({classItem})

      if(selectedClass.timeframe === 'U') return

      this.saveSelectedClass({classItem})
...
<template>
  <section>
    <article v-for="(item, idx) in classes" :key="`${idx}-${timeframe}`" :data-roster-id="item.rosterId" :data-timeframe="timeframe" v-on="handlers">
